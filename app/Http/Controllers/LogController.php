	...
	public function store(Request $request)
	{
		...
		if($student == NULL){
			...
			if($approved == 0){
				...
				$student = StudentCategories::where('cat_id', '=', $category)->firstOrFail();


                $max_allowed = $student->max_allowed;
				if($books_issued >= $max_allowed){
					return 'Student cannot issue any more books';
				} else {

					$book = Issue::where('book_id', $bookID)->first();
					...
					if($book == NULL){
						...
						if($book_availability != 1){
							...
							DB::transaction( function() use($bookID, $studentID) {
								...
								// increasing number of books issued by student
								$student = Student::find($studentID);
